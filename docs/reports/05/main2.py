# api/main.py - Mac Air 8GB Final Optimized (Timeout Fixes Applied)
import re
import requests
import chromadb
...
medical_collection = chroma_client.get_or_create_collection(name="medical_definitions_semantic",metadata={"hnsw:space": "cosine"})
...
def hybrid_rag_pipeline(query: str, eeg_docs: list[str], medical_docs: list[str]) -> tuple[str, dict]:
    ...
    medical_text = "\n".join(medical_docs[:1])[:1000] if medical_docs else ""
...
def query_eeg_data(request: QueryRequest):
    try:
        ...
        medical_results = medical_collection.query(
            query_embeddings=[query_embedding],
            n_results=min(request.n_definitions, 1),
            include=["documents"])
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "mode": "mac_air_optimized_fixed"}